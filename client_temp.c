...
//קבועים
    ...
    double flood();
    //מקבלת את כל הזוויות השונות ומנהלת עדיפות ביניהם
    void evasion();
    //פונקציה שמריצה את שתי התהליכונים זווית ומהירות בו זמנית
    void head();
...
double compute_avoidance_angle(double distance,  char* side) {
...
double Deviation_to_the_right_angle(){
    ...
    for(int i;i<length_piont;i++){
        ...
        distance=sqrt(x*x+y*y+z*z);
            //זווית של צד ימני 
            //מציאת כל הנקודות בצד הימני
            if(x>0 && distance<=2  && y<=length_car/2 || x>0 && distance<=2  && abs(y)<=length_car/2 ){
            //אם יש נקודה שהיא יותר גבוהה או יותר נמוכה
...
double back_angle=0;
    ...
    while(1){
    //מהירות מקדימה
    //זווית לנסיעה בצד ימין
    right_angle=travel_right_angle();
    Deviation_to_the_right_angle();
    //זווית מחיישן GPS
    gps_angle=Travel_by_route();
    //זווית מחיישן מים
    watet_angle=flood();
    //זווית של מכשול
    side_angle=Road_obstacle_angle();
    //זווית של מכשול בצד
    //זווית לנסיעה למקום הכי ריק
    //זווית מאחורה

//אם משהוא שלח זווית   צריך לחשב מסלול מחדש
